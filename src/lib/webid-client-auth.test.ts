...
// hardening from @jeswr/solid-session-restore @ 32bf38c (and pod-health @ d04eb33)
// into the Pod Manager's bespoke provider, closing two weaknesses the #84 work
// flagged:
//
...
describe("buildClientAuth — public clients", () => {
  it("returns `none` (no Authorization header) for a method-omitted client with NO secret", () => {
    const { headers } = applyClientAuth(buildClientAuth("https://issuer.example", client()));
    expect(headers.get("Authorization")).toBeNull();
  });

  it("returns `none` for an explicit `none` method even WITH a secret (caller chose public)", () => {
    // An EXPLICIT `none` must stay public — it's a deliberate public-client choice, never
    // upgraded to confidential just because a stray secret is present.
    const { headers, body } = applyClientAuth(
      buildClientAuth(
        "https://issuer.example",
        client({ token_endpoint_auth_method: "none", client_secret: "s3cr3t" }),
      ),
    );
    expect(headers.get("Authorization")).toBeNull();
...
describe("buildClientAuth — OMITTED method defaults per spec (roborev Medium fix)", () => {
  it("OMITTED method + NON-EMPTY secret → client_secret_basic (NOT silently downgraded to `none`)", () => {
    // OIDC Discovery / RFC 6749 §2.3.1: an omitted token_endpoint_auth_method defaults to
    // `client_secret_basic` for a confidential client. The old code returned `none` here,
    // silently dropping the secret. The secret MUST be presented via the Basic header.
    const secret = "se cret";
    const { headers, body } = applyClientAuth(
      buildClientAuth("https://pod.example/oidc", client({ client_secret: secret })),
    );
    const header = headers.get("Authorization");
    expect(header).not.toBeNull();
    expect(header?.startsWith("Basic ")).toBe(true);
    // No secret leaked in the form body — it's a Basic-header method.
    expect(body.has("client_secret")).toBe(false);
    // It is the SPEC ClientSecretBasic for a normal (non-ESS) issuer.
  ...
  it("OMITTED method + EMPTY-string secret → public `none` (not confidential)", () => {
    const { headers } = applyClientAuth(
      buildClientAuth("https://issuer.example", client({ client_secret: "" })),
    );
    expect(headers.get("Authorization")).toBeNull();
  });

  it("OMITTED method on the ESS host + secret → the BESPOKE no-url-encode Basic header", () => {
    // The omitted→client_secret_basic default still routes through the exact-host ESS
    // workaround, so an ESS confidential client with no declared method works.
    const secret = "se cret";
    const { headers } = applyClientAuth(
      buildClientAuth("https://login.inrupt.com", client({ client_secret: secret })),
    );
...
describe("buildClientAuth — FAIL-CLOSED on unsupported / missing-secret (weakness #2)", () => {
  ...
  it("(b) FAILS CLOSED on a PRESENT-but-invalid method (`null`) — not coalesced to the omitted default", () => {
    // `null` is a present (not omitted) value from malformed/untrusted metadata. A naive
    // `?? default` would treat it as omitted and accept it; it must instead fall through
    // to the unsupported-method guard and throw, both with and without a secret.
    expect(() =>
      buildClientAuth(
        "https://issuer.example",
